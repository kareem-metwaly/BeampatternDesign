"""implementations of most commonly used complex functions with pytorch it
contains implementations of matmul, pool, normalize, upsample, downsample,
relu, cosine similarity."""

import torch
from torch.nn.functional import avg_pool2d, dropout, dropout2d, interpolate, max_pool2d, relu
...
def complex_relu(input):
    """naive extention of relu, performs relu on the real and imaginary parts
    independently."""
    return relu(input.real).type(torch.complex64) + 1j * relu(input.imag).type(torch.complex64)


def _retrieve_elements_from_indices(tensor, indices):
    """get elements from the input tensor based on the indices."""
...
def complex_upsample(
    ...
    """Performs upsampling by separately interpolating the real and imaginary
    part and recombining."""
...
def complex_upsample2(
    ...
    """Performs upsampling by separately interpolating the amplitude and phase
    part and recombining."""
    ...
    return outp_abs * (torch.cos(outp_angle).type(torch.complex64) + 1j * torch.sin(outp_angle).type(torch.complex64))


def complex_max_pool2d(input, kernel_size, stride=None, padding=0, dilation=1, ceil_mode=False, return_indices=False):
    """Perform complex max pooling by selecting on the absolute value on the
    complex values."""
...
def complex_l2_norm(inp: torch.Tensor, dim: int = -1, keepdim: bool = False) -> torch.Tensor:
    """Calculates the l2 norm for a complex input through the dimension
    `dim`"""
    return torch.linalg.vector_norm(inp, ord=2, dim=dim, keepdim=keepdim)


def complex_cosine_similarity(a: torch.Tensor, b: torch.Tensor, dims: tuple[list[int], list[int]]) -> torch.Tensor:
    """Cosine Similarity between two complex valued tensors.

    Calculates the inner product (a^H.b) and then normalize it by their
    norms
    """
    return a.conj().mul(b).sum(dims=dims).div(complex_l2_norm(a).mul(complex_l2_norm(b)).clamp(min=1e-5))
